...
def _get_wav_for_path_pipeline(path, sr=8000, min_length=1.92):
    logger = get_run_logger()
    # return get_wav_for_path_pipeline(path, sr)
    signal, signal_length = get_wav_for_path_pipeline(path, sr=sr)
    if signal_length < min_length:
        raise ValueError(f"{path} too short. {signal_length} < {min_length}")
        # logger.info(f"{filename} too short. {signal_length} < {(n_hop * win_size) / sr}")
        # continue
    else:
        logger.info(f"Read {path}.  Signal Length: {signal_length}")
    
    return signal, signal_length


@flow(name="predict on frames")
...
def write_output(rootFolderPath, csv_filename, dir_out=None, det_threshold=0.5, feat_type='stft',
    ...
    for _, file_row in tqdm(files_df.iterrows(), total=len(files_df)):
        ...
        if not os.path.exists(text_output_filename):
            ...
            try:
                signal, _ = _get_wav_for_path_pipeline([os.path.join(root, filename)], sr=sr, min_length=win_size * n_hop / sr)
            except:
                signal = None
